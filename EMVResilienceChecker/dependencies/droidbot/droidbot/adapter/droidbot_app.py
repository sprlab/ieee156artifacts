...
class DroidBotAppConn(Adapter):
    ...
    def __init__(self, device=None):
        ...
        self.toasts_output_dir = os.path.join(self.device.output_dir, "states")
    ...
    def _is_toast(self, event):
        return event["class_name"] and "android.widget.Toast" in event["class_name"]

    def _save_event(self, event):
        if self.toasts_output_dir is None:
                if self.device.output_dir is None:
                    return
                else:
                    self.toasts_output_dir = os.path.join(self.device.output_dir, "toasts")
        if not os.path.exists(self.toasts_output_dir):
            os.makedirs(self.toasts_output_dir)

        with open(os.path.join(self.toasts_output_dir,f"toast_{self.tag}.json"), "w") as f:
